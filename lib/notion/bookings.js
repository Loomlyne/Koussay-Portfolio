import { bookingCopy } from "@/lib/book/confirmation";
import {
  RESEARCH_PLACEHOLDER,
  isResearchConfigured,
} from "@/lib/book/research";
import { BOOK_STEPS } from "@/lib/book/steps";
import { dataSourceId, notion } from "@/lib/notion/client";
import {
  dateRangeOf,
  findProp,
  plainText,
  rich,
  textOf,
} from "@/lib/notion/props";
import { notionBookingsDatabaseId, notionCalendarDatabaseId } from "@/lib/env";
import {
  SLOT_MS,
  allDayBusy,
  notionDateValue,
  parseNotionInstant,
  slotStartMs,
} from "@/lib/book/time";

const CLOSED_STATUSES = new Set([
  "cancelled",
  "canceled",
  "declined",
  "archived",
  "draft",
  "incomplete",
]);

const INCOMPLETE_STEPS = new Set(BOOK_STEPS.map((step) => step.slug));

function schemaKey(schema, ...names) {
  const wanted = names.map((name) => name.toLowerCase());
  return Object.keys(schema).find((name) =>
    wanted.includes(name.toLowerCase()),
  );
}

function setProp(properties, schema, names, write) {
  const key = schemaKey(schema, ...names);
  if (!key) return;
  const value = write(schema[key].type);
  if (value) properties[key] = value;
}

function asText(type, content) {
  const value = String(content ?? "").trim();
  if (!value) return null;
  if (type === "rich_text") return { rich_text: rich(value) };
  if (type === "title") return { title: rich(value) };
  if (type === "select") return { select: { name: value.slice(0, 100) } };
  if (type === "status") return { status: { name: value.slice(0, 100) } };
  if (type === "url") return { url: value };
  if (type === "email") return { email: value };
  return null;
}

function safeFilename(name) {
  const base = String(name || "attachment")
    .replace(/[/\\?%*:|"<>]/g, "-")
    .trim();
  return base.slice(0, 100) || "attachment";
}

function paragraphBlock(text) {
  return {
    object: "block",
    type: "paragraph",
    paragraph: { rich_text: rich(text) },
  };
}

function bulletBlock(text) {
  return {
    object: "block",
    type: "bulleted_list_item",
    bulleted_list_item: { rich_text: rich(text) },
  };
}

function headingBlock(level, text) {
  const type = level === 3 ? "heading_3" : "heading_2";
  return {
    object: "block",
    type,
    [type]: { rich_text: rich(text) },
  };
}

function dividerBlock() {
  return { object: "block", type: "divider", divider: {} };
}

function emailBlocks(booking) {
  const copy = bookingCopy(booking);
  return [
    paragraphBlock(copy.greeting),
    paragraphBlock(copy.when),
    paragraphBlock(copy.detailsLead),
    ...copy.details.map(([label, value]) => bulletBlock(`${label}: ${value}`)),
    paragraphBlock(copy.closing),
  ];
}

function researchPlaceholderBlocks() {
  return [
    dividerBlock(),
    headingBlock(2, "Business briefing"),
    paragraphBlock(RESEARCH_PLACEHOLDER),
  ];
}

function researchContentBlocks(briefing) {
  if (!briefing) {
    return [
      paragraphBlock(
        "Couldn't finish the research in time. Open their website before the call.",
      ),
    ];
  }

  const blocks = [paragraphBlock(briefing.summary)];
  if (briefing.whatTheyDo) {
    blocks.push(headingBlock(3, "What they do"));
    blocks.push(paragraphBlock(briefing.whatTheyDo));
  }
  if (briefing.who) {
    blocks.push(headingBlock(3, "Who it's for"));
    blocks.push(paragraphBlock(briefing.who));
  }
  if (briefing.callNotes?.length) {
    blocks.push(headingBlock(3, "For this call"));
    blocks.push(...briefing.callNotes.map((note) => bulletBlock(note)));
  }
  if (briefing.watchOuts?.length) {
    blocks.push(headingBlock(3, "Watch-outs"));
    blocks.push(...briefing.watchOuts.map((note) => bulletBlock(note)));
  }
  if (briefing.sources?.length) {
    blocks.push(headingBlock(3, "Sources"));
    blocks.push(...briefing.sources.map((note) => bulletBlock(note)));
  }
  return blocks;
}

function blockPlain(block) {
  if (!block) return "";
  if (block.type === "paragraph") return plainText(block.paragraph?.rich_text);
  if (block.type === "heading_2") return plainText(block.heading_2?.rich_text);
  if (block.type === "heading_3") return plainText(block.heading_3?.rich_text);
  return "";
}

export async function fillBookingResearch(pageId, briefing) {
  if (!pageId) return;
  const listed = await notion().blocks.children.list({
    block_id: pageId,
    page_size: 100,
  });
  const placeholder = listed.results.find(
    (block) =>
      block.type === "paragraph" && blockPlain(block) === RESEARCH_PLACEHOLDER,
  );
  const content = researchContentBlocks(briefing);
  const [first, ...rest] = content;

  if (!placeholder) {
    await notion().blocks.children.append({
      block_id: pageId,
      children: [
        dividerBlock(),
        headingBlock(2, "Business briefing"),
        ...content,
      ],
    });
    return;
  }

  if (first?.type === "paragraph") {
    await notion().blocks.update({
      block_id: placeholder.id,
      paragraph: first.paragraph,
    });
  }
  if (!rest.length) return;

  try {
    await notion().blocks.children.append({
      block_id: pageId,
      children: rest,
      after: placeholder.id,
    });
  } catch {
    await notion().blocks.children.append({
      block_id: pageId,
      children: rest,
    });
  }
}

function fileBlock(attachment, fileUploadId) {
  const name = safeFilename(attachment.filename);
  const type = String(attachment.contentType || "").toLowerCase();
  const lower = name.toLowerCase();
  const upload = {
    type: "file_upload",
    file_upload: { id: fileUploadId },
  };

  if (type.startsWith("image/") || /\.(png|jpe?g|gif|webp|avif)$/.test(lower)) {
    return { object: "block", type: "image", image: upload };
  }
  if (type === "application/pdf" || lower.endsWith(".pdf")) {
    return { object: "block", type: "pdf", pdf: upload };
  }
  return {
    object: "block",
    type: "file",
    file: { ...upload, name },
  };
}

export async function uploadNotionFile(attachment) {
  if (!attachment?.buffer || !attachment.filename) return null;

  const filename = safeFilename(attachment.filename);
  const contentType = attachment.contentType || "application/octet-stream";
  const created = await notion().fileUploads.create({
    mode: "single_part",
    filename,
    content_type: contentType,
  });
  const bytes = new Uint8Array(attachment.buffer);
  await notion().fileUploads.send({
    file_upload_id: created.id,
    file: {
      filename,
      data: new Blob([bytes], { type: contentType }),
    },
  });
  return created.id;
}

function busyRangeFromPage(page, { allowAllDay = false } = {}) {
  const status = textOf(findProp(page, "status")).toLowerCase();
  if (CLOSED_STATUSES.has(status)) return null;

  const step = textOf(findProp(page, "step", "progress")).toLowerCase();
  if (INCOMPLETE_STEPS.has(step)) return null;

  const range = dateRangeOf(findProp(page, "date"));
  const timezone =
    textOf(findProp(page, "timezone")) || range?.timeZone || "Dubai/GST";
  const time = textOf(findProp(page, "time"));
  const date = (range?.start || "").slice(0, 10);

  // The Time column is the slot the visitor picked. Prefer it over Date's
  // stored clock, which Notion Calendar can rewrite with a timezone offset.
  if (date && time) {
    const start = slotStartMs(date, time, timezone);
    if (start == null) return null;
    return { start, end: start + SLOT_MS };
  }

  if (range?.start?.includes("T")) {
    const start = parseNotionInstant(range.start, range.timeZone || timezone);
    if (start == null) return null;
    const end = range.end
      ? parseNotionInstant(range.end, range.timeZone || timezone)
      : start + SLOT_MS;
    if (end == null || end <= start) return { start, end: start + SLOT_MS };
    return { start, end };
  }

  if (!allowAllDay || !date) return null;
  return allDayBusy(date, (range?.end || "").slice(0, 10), timezone);
}

async function bookingsSource(databaseId = notionBookingsDatabaseId()) {
  const sourceId = await dataSourceId(databaseId);
  const database = await notion().dataSources.retrieve({
    data_source_id: sourceId,
  });
  return { sourceId, schema: database.properties };
}

async function fetchBusyFromDatabase(databaseId, options = {}) {
  const { sourceId, schema } = await bookingsSource(databaseId);
  const dateKey = schemaKey(schema, "date");
  const filter =
    dateKey && schema[dateKey].type === "date"
      ? {
          property: dateKey,
          date: {
            on_or_after: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000)
              .toISOString()
              .slice(0, 10),
          },
        }
      : undefined;

  const results = [];
  let startCursor;
  do {
    try {
      const page = await notion().dataSources.query({
        data_source_id: sourceId,
        start_cursor: startCursor,
        page_size: 100,
        filter,
        sorts: [{ timestamp: "created_time", direction: "descending" }],
      });
      results.push(...page.results);
      startCursor = page.has_more ? page.next_cursor : undefined;
    } catch (error) {
      if (filter) {
        return fetchBusyUnfiltered(sourceId, options);
      }
      throw error;
    }
  } while (startCursor && results.length < 300);

  return busyFromPages(results, options);
}

async function fetchBusyUnfiltered(sourceId, options = {}) {
  const results = [];
  let startCursor;
  do {
    const page = await notion().dataSources.query({
      data_source_id: sourceId,
      start_cursor: startCursor,
      page_size: 100,
      sorts: [{ timestamp: "created_time", direction: "descending" }],
    });
    results.push(...page.results);
    startCursor = page.has_more ? page.next_cursor : undefined;
  } while (startCursor && results.length < 300);
  return busyFromPages(results, options);
}

function busyFromPages(results, options = {}) {
  const busy = [];
  for (const page of results) {
    if (!page?.properties) continue;
    if (page.in_trash === true || page.archived === true) continue;
    const range = busyRangeFromPage(page, options);
    if (range) busy.push(range);
  }
  return busy;
}

let busyCache = { at: 0, ranges: null };
const BUSY_CACHE_MS = 20_000;

export function clearBusyCache() {
  busyCache = { at: 0, ranges: null };
}

export async function fetchBusyRanges() {
  if (busyCache.ranges && Date.now() - busyCache.at < BUSY_CACHE_MS) {
    return busyCache.ranges;
  }
  const bookingsId = notionBookingsDatabaseId();
  const calendarId = notionCalendarDatabaseId();
  const busy = [];
  if (bookingsId) {
    busy.push(
      ...(await fetchBusyFromDatabase(bookingsId, { allowAllDay: false })),
    );
  }
  if (calendarId && calendarId !== bookingsId) {
    busy.push(
      ...(await fetchBusyFromDatabase(calendarId, { allowAllDay: true })),
    );
  }
  busyCache = { at: Date.now(), ranges: busy };
  return busy;
}

function bookingProperties(schema, booking, fileUploadId) {
  const properties = {};
  const titleKey = Object.keys(schema).find(
    (key) => schema[key].type === "title",
  );
  if (titleKey) {
    properties[titleKey] = {
      title: rich(booking.name || booking.email || "Started a booking"),
    };
  }

  setProp(properties, schema, ["email"], (type) => asText(type, booking.email));
  if (booking.date && booking.time) {
    setProp(properties, schema, ["date"], (type) =>
      type === "date"
        ? {
            date: notionDateValue(booking.date, booking.time, booking.timezone),
          }
        : asText(type, `${booking.date} ${booking.time}`),
    );
    setProp(properties, schema, ["time"], (type) => asText(type, booking.time));
  }
  setProp(properties, schema, ["timezone"], (type) =>
    asText(type, booking.timezone),
  );
  setProp(properties, schema, ["company"], (type) =>
    asText(type, booking.company),
  );
  setProp(properties, schema, ["website", "url"], (type) =>
    asText(type, booking.website),
  );
  setProp(properties, schema, ["services", "help"], (type) =>
    type === "multi_select"
      ? {
          multi_select: (booking.services || []).map((name) => ({ name })),
        }
      : asText(type, (booking.services || []).join(", ")),
  );
  setProp(properties, schema, ["budget"], (type) =>
    asText(type, booking.budget),
  );
  setProp(properties, schema, ["deadline"], (type) =>
    asText(type, booking.deadline),
  );
  setProp(properties, schema, ["details", "notes", "about"], (type) =>
    asText(type, booking.details || ""),
  );
  setProp(
    properties,
    schema,
    ["attachment", "file", "files", "document"],
    (type) => {
      if (type === "files" && fileUploadId) {
        return {
          files: [
            {
              type: "file_upload",
              file_upload: { id: fileUploadId },
              name: safeFilename(booking.attachmentName),
            },
          ],
        };
      }
      if (type === "files") return null;
      return asText(type, booking.attachmentName || "");
    },
  );
  setProp(properties, schema, ["status"], (type) =>
    asText(type, booking.status || "New"),
  );
  setProp(properties, schema, ["step", "progress"], (type) =>
    asText(type, booking.step || ""),
  );
  return properties;
}

export async function upsertNotionDraft(draft) {
  const { sourceId, schema } = await bookingsSource();
  const properties = bookingProperties(
    schema,
    { ...draft, status: "Draft" },
    null,
  );
  const children = [
    paragraphBlock(
      `Incomplete booking. Last step: ${draft.step || "start"}. Follow up so they can finish.`,
    ),
  ];

  if (draft.id) {
    try {
      await notion().pages.update({
        page_id: draft.id,
        properties,
      });
      clearBusyCache();
      return { id: draft.id };
    } catch (error) {
      console.warn("[book] draft update failed, creating a new page", error);
    }
  }

  const created = await createBookingPage(
    sourceId,
    properties,
    children,
    draft,
  );
  clearBusyCache();
  return { id: created.id };
}

export async function createNotionBooking(booking, attachment, pageId) {
  const { sourceId, schema } = await bookingsSource();
  let fileUploadId = null;

  if (attachment) {
    try {
      fileUploadId = await uploadNotionFile(attachment);
    } catch (error) {
      console.error("[book] Notion file upload failed", error);
    }
  }

  const properties = bookingProperties(
    schema,
    { ...booking, status: "New", step: "done" },
    fileUploadId,
  );

  const children = [...emailBlocks(booking)];
  if (isResearchConfigured()) {
    children.push(...researchPlaceholderBlocks());
  }
  if (fileUploadId && attachment) {
    children.push(fileBlock(attachment, fileUploadId));
  }

  if (pageId) {
    try {
      await notion().pages.update({ page_id: pageId, properties });
      if (fileUploadId && attachment) {
        await notion().blocks.children.append({
          block_id: pageId,
          children: [fileBlock(attachment, fileUploadId)],
        });
      }
      clearBusyCache();
      return { id: pageId };
    } catch (error) {
      console.warn("[book] completing draft failed, creating a new page", error);
    }
  }

  const created = await createBookingPage(
    sourceId,
    properties,
    children,
    booking,
  );
  clearBusyCache();
  return created;
}

async function createBookingPage(sourceId, properties, children, booking) {
  try {
    return await notion().pages.create({
      parent: { data_source_id: sourceId },
      properties,
      children,
    });
  } catch {
    const fallback = Object.fromEntries(
      Object.entries(properties).filter(([, value]) => {
        const type = Object.keys(value)[0];
        return !["select", "status", "multi_select", "files"].includes(type);
      }),
    );
    try {
      return await notion().pages.create({
        parent: { data_source_id: sourceId },
        properties: fallback,
        children,
      });
    } catch {
      const schemaKeys = Object.keys(fallback);
      const dateKey = schemaKeys.find((key) => fallback[key]?.date);
      if (dateKey && fallback[dateKey]?.date?.start?.includes("T")) {
        fallback[dateKey] = { date: { start: booking.date } };
      }
      return notion().pages.create({
        parent: { data_source_id: sourceId },
        properties: fallback,
        children,
      });
    }
  }
}
