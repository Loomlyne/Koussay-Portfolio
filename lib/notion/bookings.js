...
function busyRangeFromPage(page) {
...
async function fetchBusyFromDatabase(databaseId) {
  ...
  do {
    try {
      ...
      if (filter) {
        return fetchBusyUnfiltered(sourceId);
      }
      throw error;
    }
  } while (startCursor && results.length < 300);

  return busyFromPages(results);
}

async function fetchBusyUnfiltered(sourceId) {
  const results = [];
  let startCursor;
  do {
    const page = await notion().dataSources.query({
...
function busyFromPages(results) {
  const busy = [];
  for (const page of results) {
    if (!page?.properties) continue;
    if (page.in_trash === true || page.archived === true) continue;
    const range = busyRangeFromPage(page);
...
export async function fetchBusyRanges() {
  ...
  const ids = [
    ...new Set(
      [notionBookingsDatabaseId(), notionCalendarDatabaseId()].filter(Boolean),
    ),
  ];
  const busy = [];
  for (const id of ids) {
    busy.push(...(await fetchBusyFromDatabase(id)));
  }
