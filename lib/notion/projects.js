...
function mediaPath(pageId, slot) {
  const id = notionPageId(pageId);
  return slot ? `/api/media/${id}/${slot}` : `/api/media/${id}`;
}
  ...
  return {
    ...
    detail: {
      ...
      gallery: galleryFiles.map((file, index) => ({
        file: mediaPath(page.id, `g${index}`),
...
export async function fetchNotionProjects() {
  const databaseId = await dataSourceId(notionProjectsDatabaseId());
  const results = [];
  let startCursor;
