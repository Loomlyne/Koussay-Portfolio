...
export function isHeldSlot(date, time) {
  const minutes = parseClock(time);
  const weekday = weekdayOfIso(date);
  if (minutes == null || weekday == null) return false;
  const hour = Math.floor(minutes / 60);
  return (OCCUPIED_HOURS_BY_WEEKDAY[weekday] || []).includes(hour);
}

export function isSlotOpen(date, time, timezone, busy, now = Date.now()) {
  const start = slotStartMs(date, time, timezone);
  if (start == null || start <= now) return false;
  if (isHeldSlot(date, time)) return false;
