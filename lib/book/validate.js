...
export function issueForStep(step, form, busy = []) {
  switch (step) {
    case 0:
      return hasAllFitChecks(form.fit)
        ? ""
        : "Select all three so we know we're a fit.";
    case 1:
...
export function firstInvalidStep(form, busy = [], from = 0) {
...
export function validateBooking(input) {
  ...
  const services = Array.isArray(input.services)
    ...
    : [];
  const fit = Array.isArray(input.fit)
    ? input.fit.map((item) => String(item).trim()).filter(Boolean)
    : [];

  if (!hasAllFitChecks(fit) || fit.some((item) => !FIT_KEYS.includes(item))) {
    return {
      error: "Select all three so we know we're a fit.",
      step: 0,
      field: "fit",
    };
  }
