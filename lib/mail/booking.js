...
export async function sendDraftNotice(draft) {
  if (!isResendConfigured() || !draft.email) return { skipped: true };
  ...
  const owner = await resend.emails.send({
    ...
    replyTo: draft.email,
    subject: `Booking started — ${draft.name || draft.email} · ${draft.step || "start"}`,
    html: wrap("Booking started", draft, {
      intro: `${draft.name || "Someone"} started a booking and left it on the ${draft.step || "start"} step. Follow up so they can finish.`,
      closing: "This is an incomplete draft, not a confirmed call.",
    }),
  });

  if (owner.error) {
    throw new Error(owner.error.message || "Resend draft notice failed");
  }
