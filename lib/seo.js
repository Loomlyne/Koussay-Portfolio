import { projectHref, projectShareImage, shareImageAlt } from "@/lib/projects";
import {
  BOOKING_PATH,
  SITE_DESCRIPTION,
  SITE_MARK_ALT,
...
export const SITE_SHARE_IMAGE = `${SITE_URL}${SITE_MARK_PATH}`;
export const SITE_SHARE_ALT = SITE_MARK_ALT;
...
export function shareImages(project) {
  if (project?.slug) {
    return [{ url: projectOgImage(project), alt: shareImageAlt(project) }];
  }
  const url = projectShareImage(project);
  if (url && !url.includes("/api/media/")) {
    return [{ url, alt: shareImageAlt(project) }];
  }
  return [{ url: SITE_SHARE_IMAGE, alt: SITE_SHARE_ALT }];
}
